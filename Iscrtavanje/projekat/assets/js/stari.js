...
function addKutija(){
	var b = new Box(10, 4, 6,  0.7, 0, 0, 0, "box");	
	b.Draw();
...
function dodajPregrade(){
	...
	if(pregradeVertikalno == true){
		...
		for(var k = 0; k < brojPregrada - 1; k++){
			var d = new Daska(selected.debljinaDaske, selected.visina, selected.dubina, selected.posX - selected.sirina / 2 + (k + 1) * i, selected.posY, selected.posZ, "daska" + k);
		...
		for(var k = 0; k < brojPregrada - 1; k++){
			var d = new Daska(selected.sirina, selected.debljinaDaske, selected.dubina, selected.posX, selected.posY - selected.visina / 2 + (k + 1) * i, selected.posZ, "daska" + k);
			var mesh  = d.CreateGeometry();
			//objectsOnScene.push(d);
			//var ob = scene.getObjectByName("daska" + k);
			meshes.push(mesh);
...
function dodajFioke(){
	...
	for(var i = 0; i < selected.childs.length + 1; i++){
		if(checks[i].firstChild.checked == true){
			var f = new Fioka(duz, selected.visina, selected.dubina, selected.debljinaDaske, start + (i * duz), selected.posY, selected.posZ, "Fioka" + i);
			f.Draw();
		}
	}
} 
