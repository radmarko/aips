	...
	*/
	if(pregradeVertikalno == true){
		...
		for(var k = 0; k < brojPregrada - 1; k++){
			var d = new Daska(selected.debljinaDaske, selected.visina, selected.dubina, selected.posX - selected.sirina / 2 + (k + 1) * i + startX, selected.posY + startY, selected.posZ + startZ, "daska" + k);
		...
		for(var k = 0; k < brojPregrada - 1; k++){
			var d = new Daska(selected.sirina, selected.debljinaDaske, selected.dubina, selected.posX + startX, selected.posY - selected.visina / 2 + (k + 1) * i + startY, selected.posZ + startZ, "daska" + k);
			var mesh  = d.CreateGeometry();
			//objectsOnScene.push(d);
			//var ob = scene.getObjectByName("daska" + k);
			meshes.push(mesh);
	...
	/*obj.position.x = selected.posX;
	obj.position.y = selected.posY;
	obj.position.z = selected.posZ;*/
	objects.push(obj);
	scene.add(obj);
...
function dodajFioke(){
	...
	*/
	...
	var start = selected.posX - selected.sirina / 2 + duz / 2;
	...
	for(var i = 0; i < selected.childs.length + 1; i++){
		if(checks[i].firstChild.checked == true){
			//var f = new Fioka(duz, selected.visina, selected.dubina, selected.debljinaDaske, start + (i * duz) + startX, selected.posY + startY, selected.posZ + startZ, "Fioka" + i);
			var f = new Fioka(duz, selected.visina, selected.dubina, selected.debljinaDaske, start + (i * duz) + selected.globalX, selected.posY + selected.globalY, selected.posZ + selected.globalZ, "Fioka" + i);
...
function dodajVrata(){
	...
	for(var i = 0; i < selected.childs.length + 1; i++){
		if(checks[i].firstChild.checked == true){
			var v = new Vrata(duz, selected.visina, selected.debljinaDaske, start + (i * duz) + startX, selected.posY + startY, selected.posZ + startZ + selected.dubina / 2 - selected.debljinaDaske / 2, "Vrata" + i);
